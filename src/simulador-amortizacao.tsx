...
const SimuladorAmortizacao = () => {
   ...
   */
  const analisarCapacidadePagamento = (
    prestacao: number,
    renda: number,
  ): Capacidade | null => {
    ...
    if (percentual <= 30) {
      return {
        nivel: "seguro",
        percentual,
        mensagem: "Comprometimento saudável",
        cor: "text-green-600",
        icon: CheckCircle,
      };
      ...
      return {
        nivel: "atencao",
        percentual,
        mensagem: "Comprometimento moderado",
        cor: "text-yellow-600",
        icon: AlertTriangle,
      };
    } else {
  ...
  const recomendarSistema = (
    ...
    recomendacoes.push({
      ...
      sistema: "PRICE",
      motivo: "Prestações fixas",
      peso: 1,
    });
    recomendacoes.push({
      tipo: "fluxo",
      sistema: "SAC",
      motivo: "Prestações decrescentes",
      peso: 2,
    });
    const pontosSAC = recomendacoes
      .filter((r) => r.sistema === "SAC")
      .reduce((sum, r) => sum + r.peso, 0);
    const pontosPRICE = recomendacoes
      .filter((r) => r.sistema === "PRICE")
      .reduce((sum, r) => sum + r.peso, 0);
    return {
      sistemaRecomendado: pontosSAC > pontosPRICE ? "SAC" : "PRICE",
      pontosSAC,
      pontosPRICE,
      recomendacoes,
    };
  ...
  const handleCalcular = () => {
    ...
    if (valorExtra > 0 && mesExtra > 0 && mesExtra <= carencia) {
      alert(
        `O mês da amortização extra deve ser após o período de carência (mês ${carencia + 1} ou posterior).`,
      );
      return;
    }
    ...
    if (valorExtra > 0 && mesExtra > carencia + n) {
      alert(
        `O mês da amortização extra não pode ser maior que o total de parcelas (${carencia + n}).`,
      );
      return;
    }
    ...
    if (valorExtra > P) {
      alert(
        "O valor da amortização extra não pode ser maior que o valor do empréstimo.",
      );
      return;
    }

    const parcelasSAC = calcularSAC(
      P,
      n,
      i,
      carencia,
    ...
    const parcelasPRICE = calcularPRICE(
      P,
      n,
      i,
      carencia,
    ...
    const capacidadeSAC = analisarCapacidadePagamento(
      parcelasSAC[carencia]?.prestacao || parcelasSAC[0]?.prestacao,
      renda,
    );
    const capacidadePRICE = analisarCapacidadePagamento(
      parcelasPRICE[carencia]?.prestacao || parcelasPRICE[0]?.prestacao,
      renda,
    );
    const recomendacao = recomendarSistema(
      resumoSAC,
      resumoPRICE,
      renda,
      parcelasSAC[carencia]?.prestacao || parcelasSAC[0]?.prestacao,
    ...
    setResultados({
      ...
      numParcelas: n,
      taxaJuros: i * 100,
      periodoCarencia: carencia,
      amortizacaoExtra:
        valorExtra > 0 ? { valor: valorExtra, mes: mesExtra } : null,
  ...
  const prepararDadosGraficoLinha = () => {
    ...
    return resultados.sac.parcelas.map((sac, index) => ({
      mes: sac.mes,
      SAC: sac.prestacao,
      PRICE: resultados.price.parcelas[index]?.prestacao || 0,
    }));
  };
  ...
  const prepararDadosGraficoPizza = (sistema: "sac" | "price") => {
    ...
    return [
      { name: "Juros", value: dados.resumo.totalJuros },
      { name: "Amortização", value: dados.resumo.totalAmortizacao },
    ];
  };
    ...
    return [
      {
        ...
        SAC: resultados.sac.resumo.totalPago,
        PRICE: resultados.price.resumo.totalPago,
      },
      {
        categoria: "Total Juros",
        SAC: resultados.sac.resumo.totalJuros,
        PRICE: resultados.price.resumo.totalJuros,
      },
    ];
  };
  ...
  const prepararDadosSaldoDevedor = () => {
    ...
    return resultados.sac.parcelas.map((sac, index) => ({
      mes: sac.mes,
      SAC: sac.saldoFinal,
      PRICE: resultados.price.parcelas[index]?.saldoFinal || 0,
    }));
  };
  ...
  const exportarRelatorio = () => {
    ...
    let r = "=".repeat(80) + "\n";
    r +=
      "RELATÓRIO - SISTEMAS DE AMORTIZAÇÃO\nUFMG - Administração Financeira\n" +
      "=".repeat(80) +
      "\n\n";
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                UFMG - Administração Financeira - Trabalho 2
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
                ...
                <p className="text-sm text-blue-900 mb-2">
                  Os <strong>juros são calculados sobre o saldo devedor</strong>
                  . Quanto antes você reduzir o saldo, menos juros pagará nos
                  meses seguintes!
                </p>
                <div className="bg-white rounded p-3 text-sm">
                  <p className="font-semibold mb-1">
                    Exemplo: Empréstimo de R$ 100.000 a 1% ao mês por 100 meses
                  </p>
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    <div>
                      ...
                      <p className="text-xs">
                        Economia: ~R$ 9.800 em juros (98 meses)
                      </p>
                    </div>
                    <div>
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    <strong>Diferença:</strong> Amortizar cedo economiza até{" "}
                    <strong>98x mais!</strong>
                  </p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg">
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    value={taxaInflacao}
                    onChange={(e) => setTaxaInflacao(e.target.value)}
                    className="w-full px-4 py-2 border rounded-lg"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setAmortizacaoExtra(e.target.value)}
                    className="w-full px-4 py-2 border rounded-lg"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={mesAmortizacaoExtra}
                    onChange={(e) => setMesAmortizacaoExtra(e.target.value)}
                    className="w-full px-4 py-2 border rounded-lg"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={amortizacaoMensal}
                    onChange={(e) => setAmortizacaoMensal(e.target.value)}
                    className="w-full px-4 py-2 border rounded-lg border-yellow-400 bg-yellow-50"
                    placeholder="Opcional"
                  />
                </div>
              </div>
        ...
        {resultados && (
          <>
            <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 mb-8 text-white">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h2 className="text-2xl font-bold">
                  Recomendado: {resultados.recomendacao.sistemaRecomendado}
                </h2>
              </div>
                  ...
                  <ul className="space-y-2">
                    {resultados.recomendacao.recomendacoes
                      .filter(
                        (r) =>
                          r.sistema ===
                          resultados.recomendacao.sistemaRecomendado,
                      )
                      .map((rec, i) => (
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="font-bold">
                          {resultados.recomendacao.pontosSAC} pontos
                        </span>
                      </div>
                    ...
                    <div className="border-t border-white/20 pt-2">
                      <div className="flex justify-between mb-1">
                        <span>PRICE</span>
                        <span className="font-bold">
                          {resultados.recomendacao.pontosPRICE} pontos
                        </span>
                      </div>
                      <div className="text-xs space-y-1">
                        {resultados.recomendacao.recomendacoes
            ...
            {(resultados.sac.capacidade?.nivel === "risco" ||
              resultados.price.capacidade?.nivel === "risco") && (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-red-700">
                      Alto comprometimento de renda. Considere aumentar prazo.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-bold text-green-800">
                      Amortização Extraordinária Aplicada
                    </h3>
                    <p className="text-sm text-green-700">
                      Pagamento extra de{" "}
                      <strong>
                        {formatarMoeda(resultados.amortizacaoExtra.valor)}
                      </strong>{" "}
                      no mês <strong>{resultados.amortizacaoExtra.mes}</strong>.
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700">
                      Pagando{" "}
                      <strong>
                        {formatarMoeda(resultados.amortizacaoMensal)}
                      </strong>{" "}
                      extras TODO MÊS! Esta é a estratégia MAIS EFICIENTE para
                      economizar juros e reduzir o prazo do financiamento.
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          <span>Total:</span>
                          <span className="font-bold">
                            {formatarMoeda(resultados.sac.resumo.totalPago)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Juros:</span>
                          <span className="font-bold">
                            {formatarMoeda(resultados.sac.resumo.totalJuros)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {formatarMoeda(
                              resultados.sac.parcelas[
                                resultados.periodoCarencia || 0
                              ].prestacao,
                            )}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {formatarMoeda(
                              resultados.sac.parcelas[
                                resultados.sac.parcelas.length - 1
                              ].prestacao,
                            )}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>CET Anual:</span>
                          <span className="font-bold">
                            {resultados.sac.cet.cetAnual.toFixed(2)}%
                          </span>
                        </div>
                        {resultados.sac.capacidade && (
                          <div className="mt-4 p-3 bg-white rounded-lg">
                            <div className="flex items-center gap-2">
                              <IconeCapacidade
                                capacidade={resultados.sac.capacidade}
                              />
                              <div>
                                ...
                                <p
                                  className={`text-lg font-bold ${resultados.sac.capacidade.cor}`}
                                >
                                  {resultados.sac.capacidade.percentual.toFixed(
                                    1,
                                  )}
                                  %
                                </p>
                                <p className="text-xs text-gray-600">
                                  {resultados.sac.capacidade.mensagem}
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          <span>Total:</span>
                          <span className="font-bold">
                            {formatarMoeda(resultados.price.resumo.totalPago)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Juros:</span>
                          <span className="font-bold">
                            {formatarMoeda(resultados.price.resumo.totalJuros)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Prestação:</span>
                        ...
                        <div className="flex justify-between">
                          <span>Economia vs SAC:</span>
                          <span className="font-bold">
                            {formatarMoeda(
                              Math.abs(
                                resultados.sac.resumo.totalPago -
                                  resultados.price.resumo.totalPago,
                              ),
                            )}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>CET Anual:</span>
                          <span className="font-bold">
                            {resultados.price.cet.cetAnual.toFixed(2)}%
                          </span>
                        </div>
                        {resultados.price.capacidade && (
                          <div className="mt-4 p-3 bg-white rounded-lg">
                            <div className="flex items-center gap-2">
                              <IconeCapacidade
                                capacidade={resultados.price.capacidade}
                              />
                              <div>
                                <p className="text-sm font-medium">
                                  Comprometimento
                                ...
                                <p
                                  className={`text-lg font-bold ${resultados.price.capacidade.cor}`}
                                >
                                  {resultados.price.capacidade.percentual.toFixed(
                                    1,
                                  )}
                                  %
                                </p>
                                <p className="text-xs text-gray-600">
                                  {resultados.price.capacidade.mensagem}
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <ResponsiveContainer width="100%" height={250}>
                          <RePieChart>
                            <Pie
                              data={prepararDadosGraficoPizza("sac")}
                              cx="50%"
                              cy="50%"
                              labelLine={false}
                              label={({ name, percent }) =>
                              ...
                              dataKey="value"
                            >
                              {prepararDadosGraficoPizza("sac").map(
                                (_, index) => (
                                  <Cell
                                    key={`cell-${index}`}
                      ...
                      <div>
                        ...
                        <ResponsiveContainer width="100%" height={250}>
                          <RePieChart>
                            <Pie
                              data={prepararDadosGraficoPizza("price")}
                              cx="50%"
                              cy="50%"
                              labelLine={false}
                              label={({ name, percent }) =>
                              ...
                              dataKey="value"
                            >
                              {prepararDadosGraficoPizza("price").map(
                                (_, index) => (
                                  <Cell
                                    key={`cell-${index}`}
                                    fill={CORES[index % CORES.length]}
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span>CET Mensal:</span>
                              <span className="font-medium">
                                {resultados.sac.cet.cetMensal.toFixed(2)}%
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>CET Anual:</span>
                            ...
                            <div className="flex justify-between">
                              <span>TIR:</span>
                              <span className="font-medium">
                                {resultados.sac.tir.toFixed(2)}%
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>VPL:</span>
                              <span className="font-medium">
                                {formatarMoeda(resultados.sac.vpl)}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span>CET Mensal:</span>
                              <span className="font-medium">
                                {resultados.price.cet.cetMensal.toFixed(2)}%
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>CET Anual:</span>
                            ...
                            <div className="flex justify-between">
                              <span>TIR:</span>
                              <span className="font-medium">
                                {resultados.price.tir.toFixed(2)}%
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>VPL:</span>
                              <span className="font-medium">
                                {formatarMoeda(resultados.price.vpl)}
                              </span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="bg-blue-50 rounded-lg p-6">
                      ...
                      <div className="space-y-3 text-sm text-gray-700">
                        <p>
                          <strong>CET:</strong> Custo efetivo total incluindo
                          juros, IOF, tarifas e seguros.
                        </p>
                        <p>
                          <strong>TIR:</strong> Taxa interna de retorno da
                          operação.
                        </p>
                        <p>
                          <strong>VPL:</strong> Valor presente líquido das
                          prestações.
                        </p>
                      </div>
                    </div>
